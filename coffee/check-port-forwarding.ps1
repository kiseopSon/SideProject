...
if ($listening) {
    Write-Host "   ✅ 서버가 8080 포트에서 리스닝 중입니다." -ForegroundColor Green
    Write-Host "   $listening" -ForegroundColor Gray
} else {
    Write-Host "   ❌ 서버가 8080 포트에서 리스닝하지 않습니다." -ForegroundColor Red
}
Write-Host ""

# 2. 로컬 네트워크 접속 테스트
Write-Host "2. 로컬 네트워크 접속 테스트 (192.168.0.3:8080)..." -ForegroundColor Yellow
try {
    $response = Test-NetConnection -ComputerName 192.168.0.3 -Port 8080 -WarningAction SilentlyContinue
...
$firewallRule = netsh advfirewall firewall show rule name="Spring Boot Gateway (8080)" 2>$null
if ($firewallRule -match "확인됨|Ok") {
    ...
    Write-Host "   netsh advfirewall firewall add rule name=`"Spring Boot Gateway (8080)`" dir=in action=allow protocol=TCP localport=8080" -ForegroundColor Gray
}
Write-Host ""
...
try {
    ...
    Write-Host "   외부 접속 URL: http://$publicIP:8080" -ForegroundColor Cyan
} catch {
...
Write-Host "  □ 외부 포트와 내부 포트가 모두 8080인가?" -ForegroundColor White
Write-Host "  □ 프로토콜이 TCP 또는 Both로 설정되어 있는가?" -ForegroundColor White
Write-Host "  □ 라우터를 재부팅했는가?" -ForegroundColor White
Write-Host "  □ 같은 네트워크의 다른 기기에서 192.168.0.3:8080 접속이 되는가?" -ForegroundColor White
Write-Host ""
